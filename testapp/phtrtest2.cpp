...
bool parse_command_line(int argc, char* argv[], Parameters& params)
{
    ...
    try
    {
        ...
        {
            params.param_crop = options_map["param-crop"].as<double>();
        }

        if (options_map.count("param-aspect"))
        {
            params.param_aspect = options_map["param-aspect"].as<double>();
        }

        if (options_map.count("image-crop"))
        {
            params.image_crop = options_map["image-crop"].as<double>();
        }

        if (options_map.count("input-file"))
        {
            params.inp_file = options_map["input-file"].as<std::string>();
        }
        else
        {
        ...
        {
            params.outp_file = options_map["output-file"].as<std::string>();
        }
        else
        {
        ...
        if (options_map.count("ptlens"))
        {
            ...
            for (tokenizer_t::const_iterator it = tokens.begin(); it != tokens.end(); ++it)
            {
                ...
                sstr >> params.ptlens_params[param_idx++];
            }
            ...
            std::cerr << "Performing PTLens correction: "
                      << params.ptlens_params[0] << ":"
                      << params.ptlens_params[1] << ":"
                      << params.ptlens_params[2] << ":"
                      << params.ptlens_params[3] << ":"
                      << params.ptlens_params[4] << std::endl;
        }
...
void convert_8bit(const Parameters& params)
{
        ...
        vil_convert_to_component_order(
            vil_convert_to_n_planes(
                3, vil_convert_stretch_range(
                    vil_channel_t(), vil_load(params.inp_file.c_str())
                )
            )
        );
    ...
    if (params.ptlens_corr)
    {
        PTLensGeomModel ptlens_mod(params.param_aspect,
                                   image_aspect,
                                   params.param_crop,
                                   params.image_crop);
        ptlens_mod.set_model_params(params.ptlens_params[0],
                                    params.ptlens_params[1],
                                    params.ptlens_params[2],
                                    1.0,
                                    params.ptlens_params[3],
                                    params.ptlens_params[4]);
    ...
    vil_save(vil_dst_view, params.outp_file.c_str());
}
